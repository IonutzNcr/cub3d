...
/*   Updated: 2025/11/05 19:40:08 by inicoara         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_walls()
{
	...
	player = *sgt_player();
	isOut = 0;
	ft_searchpath(player.x, player.y, map_tmp, &isOut);
	while (!isOut)
	{
	...
	if (isOut && set_sgt_error(NO_WALL))
		return (*sgt_error());
	return (0);
}
...
void	ft_searchpath(int x, int y, char **map, int *isOut)
{
	while (1)
	{
		if ((map[y][x] == '-' || is_on_edge(x,y)) && ++(*isOut))
			break;
		if (map[y][x] == '1')
			break;
		if (can_move(x, y, map))
		...
		if (can_move(x + 1, y, map))
			ft_searchpath(x + 1, y, map, isOut);
		if (can_move(x - 1, y, map))
			ft_searchpath(x - 1, y, map, isOut);
		if (can_move(x, y + 1, map))
			ft_searchpath(x, y + 1, map, isOut);
		if (can_move(x, y - 1, map))
			ft_searchpath(x, y - 1, map, isOut);
		break ;
	}

...
/*   Updated: 2024/11/27 18:23:03 by inicoara         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	error = 0;
	rd = 1;
	while (!error && rd && ft_index_line(ptr, '\n') == -1)
		ptr = ft_read_once(ptr, fd, &rd, &error);
	if (ft_index_line(ptr, '\n') != -1)
	{
		line = ft_get_line(ptr);
		if (!line)
			return (ft_free(&ptr, 0));
		ptr = ft_substract(ptr, ft_index_line(ptr, '\n') + 1, &error);
		if (!ptr && error)
